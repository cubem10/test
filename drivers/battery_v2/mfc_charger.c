...
void mfc_fod_set(struct mfc_charger_data *charger)
{
	...
	case SEC_WIRELESS_PAD_WPC_HV_20:
...
static void mfc_wpc_afc_vout_work(struct work_struct *work)
{
	...
	if (charger->pdata->cable_type == SEC_WIRELESS_PAD_WPC_HV_20 ||
		charger->pdata->cable_type == SEC_WIRELESS_PAD_WPC_DUO_HV_20 ||
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_EMPTY:
		...
		tmp = tmp >> 7;
		pr_info("%s: MFC LDO (%s), vout (%d)\n", __func__, (val->intval == MFC_LDO_ON ? "ON" : "OFF"), tmp);
		if ((val->intval == MFC_LDO_ON) && !tmp) { /* LDO ON */
		...
		} else if ((val->intval == MFC_LDO_OFF) && tmp) { /* LDO OFF */
			...
			if(charger->pdata->cable_type != SEC_WIRELESS_PAD_NONE) {
				if(charger->adt_transfer_status != val->intval) {
					...
					if(charger->adt_transfer_status == WIRELESS_AUTH_PASS) {
						...
						charger->pdata->cable_type = SEC_WIRELESS_PAD_WPC_DUO_HV_20_LIMIT;
					} else /* when auth is failed, cable type is HV wireless */
						charger->pdata->cable_type = SEC_WIRELESS_PAD_WPC_HV;
					wake_lock(&charger->wpc_afc_vout_lock);
					queue_delayed_work(charger->wqueue, &charger->wpc_afc_vout_work, msecs_to_jiffies(0));
				} else
					pr_info("%s: skip a same pass/fail result\n", __func__);
			}
#endif
			break;
