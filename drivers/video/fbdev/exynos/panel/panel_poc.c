...
int poc_erase(struct panel_device *panel, int addr, int len, int sz_block)
{
	struct panel_poc_device *poc_dev = &panel->poc_dev;
	struct panel_poc_info *poc_info = &poc_dev->poc_info;
	int ret, i, count, erase_seq_index;

	if (sz_block % POC_PAGE || sz_block > SZ_64K) {
		pr_err("%s, invalid block size %d\n", __func__, sz_block);
		return -EINVAL;
	}
...
int set_panel_poc(struct panel_poc_device *poc_dev, u32 cmd, const char *cmd_ext)
{
	...
	case POC_OP_SECTOR_ERASE:
			...
			ret = poc_erase(panel, addr, len, SZ_64K);
			if (unlikely(ret < 0)) {
